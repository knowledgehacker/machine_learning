...
LogisticRegression::LogisticRegression(const Samples<float>& samples, const float a, const RegressionFunction& regFunc, float thres): 
	trainingSamples(samples), featureNum(samples.getFeatureNum()), sampleNum(samples.getSampleNum()), alpha(a), rf(regFunc), threshold(thres) {
...
int LogisticRegression::gradientAscent() {
	...
	do {
		...
		/*
		 * Big problem: how to handle the case that 'z' to sigmoid(z) is either too big or too small???
		 */
		// calculate deviations for all samples, one per sample.
		for(int i = 0; i < sampleNum; ++i)
			deviations[i] = categories[i] - rf(trainingSamples.getSample(i), parameters, featureNum);
 ...
 * 1) For each update of parameters, only one sample instead of all samples is involved, which reduces computation greatly.
 * 2) It's an online, since when new samples come, parameters can be updated to reflect their impact.
 */
...
bool LogisticRegression::convergent(const long double* deviations, const float categoryRange)const {
	for(int i = 0; i < sampleNum; ++i)
		if((fabs(deviations[i]) / categoryRange) >= threshold) {
#ifdef LR_DEBUG
		std::cout<<"i = "<<i<<", fabs(deviations[i]) / categoryRange = "<<fabs(deviations[i]) / categoryRange<<std::endl;
#endif
			return false;
		}
